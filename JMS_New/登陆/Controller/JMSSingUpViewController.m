...
@interface JMSSingUpViewController ()
...
//验证码
@property (nonatomic, strong) JMSInPutTextField *codeTextField;

//获取验证码按钮
@property (nonatomic, strong) JMSCountDownButton *countDownBtn;
...
//模型数组
@property (nonatomic,strong) NSArray<JMSHobbyModel *> *hobbyModelsArray;


@end

@implementation JMSSingUpViewController
...
-(void)viewDidADisappear:(BOOL)animated{
...
//    //用户许可协议按钮
//    protocolLabel.backgroundColor = [UIColor whiteColor];
//    protocolLabel.text = @"注册即代表您同意";
...
//        }
//        //数组
//        self.hobbyModelsArray = [JMSHobbyModel mj_objectArrayWithKeyValuesArray:response[@"hobbys"]];
//        
//        DebugLog(@"%@",response);
//        //前往主页面
//        [self `Information];
...
- (void)gotoAddInformation {
    JMSMeViewController *addInformationViewController = [[JMSMeViewController alloc]init];
    //手机号码
...
- (JMSInPutTextField *)phoneTextField {
    if(!_phoneTextField) {
        _phoneTextField = [JMSInPutTextField inputTextFieldWithFrame:CGRectMake(14, 139, SCREEN_WIDTH - 14, 33) WithPlaceholder:@"手机号码" delegate:self];
...
//验证码textField
- (JMSInPutTextField *)codeTextField {
    if(!_codeTextField) {
        _codeTextField = [JMSInPutTextField inputTextFieldWithFrame:CGRectMake(0, 139, SCREEN_WIDTH, 33) WithPlaceholder:@"验证码" delegate:self];
...
//验证码按钮
- (JMSCountDownButton *)countDownBtn {
    if(!_countDownBtn) {
        _countDownBtn = [[JMSCountDownButton alloc]init];
        ...
        [_countDownBtn countDownButtonHandler:^(JMSCountDownButton*sender, NSInteger tag) {
            ...
            [sender countDownChanging:^NSString *(JMSCountDownButton *countDownButton,NSUInteger second) {
                NSString *title = [NSString stringWithFormat:@"剩余%zd秒",second];
                return title;
            }];
            [sender countDownFinished:^NSString *(JMSCountDownButton *countDownButton, NSUInteger second) {
...
//密码textField
- (JMSInPutTextField *)passwordTextField {
    if(!_passwordTextField) {
        _passwordTextField = [JMSInPutTextField inputTextFieldWithFrame:CGRectMake(0, 139, SCREEN_WIDTH, 33) WithPlaceholder:@"请设置6-20位登录密码" delegate:self];
